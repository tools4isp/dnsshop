	...
	$content .= '<secDNS:add>
		...
		foreach($dnskeys as $dnskey){
			if(isset($dnskey) && $dnskey !== ''){
				...
				$content .= '<secDNS:dsData>
				<secDNS:keyTag>'.$valuetmp[0].'</secDNS:keyTag>
				<secDNS:alg>'.$valuetmp[1].'</secDNS:alg>
				<secDNS:digestType>'.$valuetmp[2].'</secDNS:digestType>
				<secDNS:digest>'.$valuetmp[3].'</secDNS:digest>
				</secDNS:dsData>
				';
				unset($valuetmp);
			}
...
if($query->num_rows == "0"){
	...
	while($row = $query->fetch_array(MYSQLI_ASSOC)){
		...
		if(is_array($dnskey)){
			...
			if(!isset($arrayinfo['epp']['response']['extension']['secDNS:infData']['secDNS:dsData']) && $do_dnssec === 1){
				$status = sidn_insertdnssec($row['name'],$ds,$connect);
				$eppdone++;
				var_dump($row['name']);
			}
